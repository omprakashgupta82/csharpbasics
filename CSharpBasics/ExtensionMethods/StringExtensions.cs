...
public static class StringExtensions
{
    ...
    /// <summary>
    /// Converts a number in words representation
    /// </summary>
    /// <param name="number"></param>
    /// <returns></returns>
    public static string NumberToWords(this int number, string format)
    {
        string words="";
        if (number == 0)
            return "zero";

        if (format.ToUpper() == "US")
        {
            if (number < 0)
                return "minus " + NumberToWords(Math.Abs(number), format);

            words = "";

            if ((number / 1000000) > 0)
            {
                words += NumberToWords(number / 1000000, format) + " million ";
                number %= 1000000;
            }

            if ((number / 1000) > 0)
            {
                words += NumberToWords(number / 1000, format) + " thousand ";
                number %= 1000;
            }

            if ((number / 100) > 0)
            {
                words += NumberToWords(number / 100, format) + " hundred ";
                number %= 100;
            }
        ...
        else if (format.ToUpper() == "IND")
        {
            ...
            {
                words += NumberToWords(number / 10000000, format) + " crore ";
                number %= 10000000;
            }
            if ((number / 100000) > 0)
            {
                words += NumberToWords(number / 100000, format) + " lac ";
                number %= 100000;
            }
